...
export function VolumeProfilePanel() {
  ...
  const levels = useMemo(() => {
    if (!profile) return [];
    return [...profile.levels].sort((a, b) => b.total_vol - a.total_vol);
  }, [profile]);
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="px-4 py-2 text-sm font-semibold text-text/80 border-b border-border shrink-0 flex items-center justify-between gap-2">
        ...
        {profile ? (
          <span className="text-[11px] font-mono text-text/50 uppercase">
            {profile.period}
          </span>
        ) : null}
      </div>
        ...
        {!profile ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-2 text-xs font-mono">
              <MetricCard label="POC" value={formatPrice(profile.poc)} tone="orange" />
              <MetricCard label="VAH" value={formatPrice(profile.vah)} tone="cyan" />
              <MetricCard label="VAL" value={formatPrice(profile.val)} tone="rose" />
              <MetricCard label="Total" value={formatVolume(profile.total_vol)} tone="slate" />
            </div>
            ...
            <div className="rounded border border-border/60 bg-bg/40 p-3">
              ...
              <div className="space-y-2">
                {levels.slice(0, 12).map((level) => {
                  ...
                  const isVah = level.price === profile.vah;
                  const isVal = level.price === profile.val;
                  const tone = isPoc ? "bg-orange-400" : isVah ? "bg-cyan-400" : isVal ? "bg-rose-400" : "bg-white/70";
                  return (
                    <div key={level.price} className="space-y-1">
                      ...
                      <div className="h-2 rounded bg-white/5 overflow-hidden">
                        <div
                          ...
                          style={{ width: `${Math.max(2, Math.min(100, Math.round(level.pct_of_max * 100)))}%` }}
                        />
                      </div>
                      {isPoc || isVah || isVal ? (
